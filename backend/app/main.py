...
app = FastAPI(title="HackAlem Smart Contractor Matcher", version="1.0.0")
...
def health() -> dict[str, object]:
    return {"status": "ok", "profiles_loaded": len(CONTRACTORS)}


@app.get("/api/meta", response_model=CatalogMeta)
def meta() -> CatalogMeta:
    return CatalogMeta(profile_count=len(CONTRACTORS), cities=unique_values(CONTRACTORS, "city"), categories=unique_values(CONTRACTORS, "categories"), event_formats=unique_values(CONTRACTORS, "event_formats"), languages=unique_values(CONTRACTORS, "languages"))
...
def recommendations(request: RecommendationRequest) -> RecommendationResponse:
    ...
    return recommend(request, CONTRACTORS)
