...
const COPY = {
  ru: {
    ...
    matcher: { eyebrow: 'Точный подбор без лишнего шума', title: 'Подрядчики под ваше событие', subtitle: 'До трёх проверенных вариантов с понятным объяснением выбора' },
    home: { badge: 'Сервис подбора подрядчиков', titleA: 'Соберите сильную команду', titleB: 'для вашего события', lead: 'HelpHunter сравнивает условия, бюджет и доступность — и показывает подходящих специалистов без бесконечного поиска.', primary: 'Начать подбор', secondary: 'Как это работает', trust: '66 профилей в каталоге', speed: 'Подбор за секунды', explain: 'Прозрачный результат', panelTitle: 'Ваше событие', panelStatus: 'Подбор готов', panelCity: 'Алматы', panelType: 'Корпоратив', panelResult: '3 подходящих подрядчика', sectionEyebrow: 'Простой процесс', sectionTitle: 'От идеи до готовой команды', steps: [['01', 'Опишите событие', 'Укажите город, дату, формат и комфортный бюджет.'], ['02', 'Получите подборку', 'Алгоритм проверит каталог и объяснит каждый результат.'], ['03', 'Соберите смету', 'Добавьте специалистов в список и оцените стартовый бюджет.']], categoriesTitle: 'Всё для события — в одном месте', categoriesText: 'Ведущие, декораторы, фотографы и другие специалисты из единого каталога.' },
...
export default function App({ client = api }) {
  ...
  const [language, setLanguage] = useState(() => getStored('helphunter-language', 'ru'));
  const [theme, setTheme] = useState(() => getStored('helphunter-theme', window.matchMedia?.('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'));
  const [cart, setCart] = useState(() => { try { return JSON.parse(localStorage.getItem('helphunter-cart') || '[]'); } catch { return []; } });
  ...
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem('helphunter-theme', theme); }, [theme]);
  useEffect(() => { document.documentElement.lang = language; localStorage.setItem('helphunter-language', language); }, [language]);
  useEffect(() => { localStorage.setItem('helphunter-cart', JSON.stringify(cart)); }, [cart]);
  ...
  return <div className="site-shell">
    ...
    </aside><ResultsPanel language={language} state={state} dirty={dirty} onRetry={() => submit(state.request)} onAddToCart={addToCart} cartIds={cartIds} onOpenCart={() => navigate('cart')} /></div></main>}
