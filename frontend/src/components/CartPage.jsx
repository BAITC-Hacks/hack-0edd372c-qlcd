...
export default function CartPage({ copy, items, onBrowse, onRemove, onClear, language = 'ru' }) {
  ...
  const plural = new Intl.PluralRules('ru').select(items.length);
  const unit = language === 'en' ? (items.length === 1 ? 'contractor' : 'contractors') : language === 'kk' ? 'мердігер' : ({ one: 'подрядчик', few: 'подрядчика', many: 'подрядчиков', other: 'подрядчика' }[plural]);
